        ...
        result = await send('Runtime.evaluate', {
            ...
            (function() {
                ...
                items.forEach(function(item) {
                    var link = item.querySelector("a[href*=\'/explore/\']");
                    ...
                    for (var i = 0; i < spans.length; i++) {
                        if (spans[i].innerText.indexOf("收藏") >= 0) {
                            var prev = spans[i - 1];
                            if (prev) collect = parseInt(prev.innerText.replace(/\\D/g, "")) || 0;
                            break;
                        }
                    }
                    if (link) {
                        var m = link.href.match(/\\/explore\\/([a-f0-9]+)/);
                        if (m) {
                            results.push({
                                id: m[1],
                                title: titleEl ? titleEl.innerText.substring(0, 50) : "（无标题）",
                                collect: collect
                            });
                        }
                    }
